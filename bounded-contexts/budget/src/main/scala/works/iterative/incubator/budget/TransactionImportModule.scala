...
object TransactionImportModule:
  /** Layer containing the mock presenter for UI development.
    *
    * This layer provides a standalone mock implementation that doesn't depend on any
    * domain services, useful for developing UI components in isolation.
    */
  ...
  /** Layer containing the in-memory implementation for testing and development.
    *
    * This layer provides a fully functional implementation using in-memory repositories
    * and a mock bank service.
    */
  val inMemoryLayer: ULayer[TransactionImportPresenter] =
    ZLayer.make[TransactionImportPresenter](
      ...
      MockFioBankService.layer
    )

  /** Layer for production use (not fully implemented yet).
    *
    * This layer would use real infrastructure implementations for repositories
    * and bank service integration.
    *
    * TODO: Replace in-memory repositories with real implementations.
    * TODO: Replace mock bank service with real Fio Bank API integration.
    */
  val liveLayer: ULayer[TransactionImportPresenter] =
    // For now, this is the same as inMemoryLayer until we have real implementations
    inMemoryLayer