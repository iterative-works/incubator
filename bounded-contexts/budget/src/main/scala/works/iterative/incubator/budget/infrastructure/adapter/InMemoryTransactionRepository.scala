...
/** In-memory implementation of TransactionRepository for testing and development.
  *
  * Stores transactions in memory without persistence. Uses ZIO Ref for thread-safe concurrent
  * access.
  *
...
final case class InMemoryTransactionRepository(
    ...
    /** Saves a transaction. Uses atomic update to ensure thread-safety.
      *
      * @param transaction
    ...
    /** Saves multiple transactions in a single atomic operation. This ensures that either all
      * transactions are saved or none are (atomicity).
      *
      * @param transactions
    ...
    /** Finds a transaction by ID.
      ...
      * @return
      *   A ZIO effect that completes with the found transaction or None if not found
      */
    ...
    /** Finds transactions for an account within a date range.
      *
      * @param accountId
      *   The account ID to find transactions for
      * @param startDate
      *   The start date of the range
      * @param endDate
      *   The end date of the range
      * @return
      *   A ZIO effect that completes with a list of found transactions
      */
    ...
    /** Finds transactions for a specific import batch.
      *
      * @param importBatchId
      *   The import batch ID to find transactions for
      * @return
      *   A ZIO effect that completes with a list of found transactions
      */
    ...
    /** Updates the status of all transactions in a specific import batch. This operation is atomic
      * \- either all transactions are updated or none are.
      *
      * @param importBatchId
      *   The import batch ID to update transactions for
      * @param status
      *   The new status to set
      * @return
      *   A ZIO effect that completes with the number of transactions updated
      */
    ...
    /** Counts transactions with a specific status.
      *
      * @param status
      *   The status to count transactions for
      * @return
      *   A ZIO effect that completes with the count
      */
