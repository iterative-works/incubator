...
trait BankTransactionService:
  /** Validates a date range for the specified account.
    *
    * This method is account-aware and can apply different validation rules
    * based on the bank associated with the account.
    *
    * @param accountId
    ...
    * @return
    *   A ZIO effect that completes successfully if the date range is valid for this account,
  ...
  /** Basic date range validation that applies to all banks.
    ...
    *   The end date of the range
    * @return
    *   A ZIO effect that completes successfully if the basic validation passes,
    *   or fails with an InvalidDateRange error if invalid
    */
  ...
  /** Bank-specific date range validation.
    *
    * This method should be overridden by implementations to provide
  ...
  /** Validates a date range without account context (backward compatibility).
    ...
    *   The end date of the range
    * @return
    *   A ZIO effect that completes successfully if basic validation passes,
    *   or fails with an InvalidDateRange error if invalid
    */
...
object BankTransactionService:
  /** Validates a date range for a specific account.
    ...
    *   range is valid for the specified account, or fails with an InvalidDateRange error if invalid
    */
  def validateDateRangeForAccount(
      accountId: AccountId,
      startDate: LocalDate,
