...
/** Component for the transaction import form. Provides a unified form that handles all validation
  * centrally.
  *
    ...
    def render(viewModel: TransactionImportFormViewModel): Frag =
        frag(
            ...
            form(
                ...
                div(
                    ...
                    DateRangeSelector.render(
                        DateRangeSelectorViewModel(
                            ...
                            validationError =
                                if viewModel.fieldErrors.contains("startDate") then
                                    viewModel.fieldErrors.get("startDate")
                                else if viewModel.fieldErrors.contains("endDate") then
                                    viewModel.fieldErrors.get("endDate")
                                else if viewModel.fieldErrors.contains("dateRange") then
                                    viewModel.fieldErrors.get("dateRange")
                                else None
                        )
                    )
                ),
                ...
                (if viewModel.importStatus != ImportStatus.NotStarted && !viewModel.isSubmitting
                 then
                     div(
