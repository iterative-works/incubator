...
object StatusIndicator:
    ...
    def render(viewModel: StatusIndicatorViewModel): Frag =
        ...
        if !viewModel.isVisible then
            ...
            div(
                id := "status-indicator",
                cls := s"rounded-md px-4 py-3 flex items-center w-full ${statusContainerClass(viewModel)}",
                ...
                div(cls := "icon-container relative w-5 h-5 flex-shrink-0 mr-1.5")(
                    // Not Started icon - only visible when in this state
                    div(
                        cls := s"absolute inset-0 ${
                                if viewModel.status == ImportStatus.NotStarted then "opacity-100"
                                else "opacity-0"
                            }",
                        attr("data-icon") := "not-started"
                    )(
                        raw(s"""<svg class="h-5 w-5 text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    div(
                        cls := s"absolute inset-0 transition-opacity duration-300 ${
                                if viewModel.status == ImportStatus.InProgress then "opacity-100"
                                else "opacity-0"
                            }",
                        attr("data-icon") := "in-progress"
                    )(
                        raw(s"""<svg class="h-5 w-5 text-blue-600 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    div(
                        cls := s"absolute inset-0 transition-opacity duration-300 ${
                                if viewModel.status == ImportStatus.Completed then "opacity-100"
                                else "opacity-0"
                            }",
                        attr("data-icon") := "completed"
                    )(
                        raw(s"""<svg class="h-5 w-5 text-green-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    div(
                        cls := s"absolute inset-0 transition-opacity duration-300 ${
                                if viewModel.status == ImportStatus.Error then "opacity-100"
                                else "opacity-0"
                            }",
                        attr("data-icon") := "error"
                    )(
                        raw(s"""<svg class="h-5 w-5 text-red-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
