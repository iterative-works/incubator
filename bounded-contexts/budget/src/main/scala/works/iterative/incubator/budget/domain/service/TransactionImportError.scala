...
object TransactionImportError:
  ...
  /** Error indicating a failure in the bank API connection.
    ...
    */
  case class BankApiError(message: String, cause: Option[Throwable] = None) extends TransactionImportError
  ...
  /** Error indicating no transactions were found for the date range.
    ...
    */
  case class NoTransactionsFound(startDate: LocalDate, endDate: LocalDate) extends TransactionImportError
  ...
  /** Error indicating a failure in saving the imported transactions.
    ...
    *   Optional underlying exception that caused the failure
    */
  case class TransactionStorageError(message: String, cause: Option[Throwable] = None) extends TransactionImportError
  ...
  /** Error indicating issues with an import batch.
    ...
    *   Optional underlying exception that caused the failure
    */
  case class ImportBatchError(message: String, cause: Option[Throwable] = None) extends TransactionImportError
  ...
  /** General error for any other unexpected issues.
    ...
    */
  case class UnexpectedError(message: String, cause: Option[Throwable] = None) extends TransactionImportError