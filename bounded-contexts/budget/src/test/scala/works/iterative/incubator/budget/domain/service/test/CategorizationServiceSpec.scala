...
object CategorizationServiceSpec extends ZIOSpecDefault:
    ...
    val automatedCategorizationTests = suite("Automated Categorization")(
        test("should categorize transactions based on content") {
            for
                ...
                restaurantTx =
                    createTestTransaction(accountId, "tx-restaurant", "Restaurant Payment")
                ...
                _ <- env.processingStateRepo.save(
                    groceryTx.id,
                    TransactionProcessingState.initial(groceryTx)
                )
                _ <- env.processingStateRepo.save(
                    restaurantTx.id,
                    TransactionProcessingState.initial(restaurantTx)
                )
                _ <- env.processingStateRepo.save(
                    unknownTx.id,
                    TransactionProcessingState.initial(unknownTx)
                )
        ...
        test("should categorize a single transaction") {
            for
                ...
                _ <- env.processingStateRepo.save(
                    coffeeTx.id,
                    TransactionProcessingState.initial(coffeeTx)
                )
    ...
    private def makeCategorizationService(env: MockFactory.MockEnvironment)
        : UIO[CategorizationService] =
        ...
        val categorizationStrategy = new CategorizationStrategy:
            def categorize(transaction: Transaction): UIO[TransactionCategorization] =
                ...
                env.categorizationProvider.categorizeTransaction(transaction)
                    .fold(
                        _ =>
                            works.iterative.incubator.budget.domain.service.TransactionCategorization(
