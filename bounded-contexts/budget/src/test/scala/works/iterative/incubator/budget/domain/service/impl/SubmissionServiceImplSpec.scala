...
object SubmissionServiceImplSpec extends ZIOSpecDefault:
    ...
    class TestYnabSubmitter extends YnabSubmitter:
        def submitTransaction(request: YnabSubmissionRequest): UIO[Either[String, YnabSubmissionResponse]] =
            ...
            ZIO.succeed {
                ...
                val transactionId = s"ynab-tx-${Math.abs(request.payeeName.hashCode()).toString.take(8)}"
        ...
        def init(transactions: Map[TransactionId, Transaction]): UIO[Unit] = 
            ZIO.succeed {
                ...
                val entries = transactions.toList.zipWithIndex.map { case ((id, tx), idx) =>
                    if idx == 0 then // First transaction is already categorized and ready for submission
    ...
    override def spec =
        suite("SubmissionServiceImpl")(
            test("submitTransactions should process valid transactions and mark them as Submitted") {
                for
                    env <- testEnvironment
