...
import works.iterative.incubator.budget.domain.query.{TransactionProcessingStateQuery, CategoryQuery}
        ...
        suite("CategorizationServiceImpl")(
            test("categorizeTransactions should assign categories with confidence scores") {
                for
                    ...
                    categorizedStates = states.values.filter(_.status == TransactionStatus.Categorized)
