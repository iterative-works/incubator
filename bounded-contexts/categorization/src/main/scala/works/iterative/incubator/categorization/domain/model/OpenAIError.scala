...
/** Domain-specific errors related to OpenAI operations
  *
  * This error hierarchy defines all the domain-relevant error types that can occur when interacting
  * with OpenAI API for payee cleanup operations.
  */
...
object OpenAIError:
    ...
    case class ResponseParsingError(message: String, cause: Option[Throwable] = None)
        extends OpenAIError:
