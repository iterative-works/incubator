...
trait OpenAIClient:
    /** Clean up a payee name using OpenAI's language models
      *
      * @param original The original messy payee name
      * @param context Additional transaction context like counterAccount, message, etc.
      * @return An effect that produces a cleaned payee name and optionally a suggested rule
      */
    def cleanupPayee(
        original: String,
    ...
    /** Check if the OpenAI client is configured and can connect to API
      *
      * @return An effect that completes successfully if the client is healthy
      */
