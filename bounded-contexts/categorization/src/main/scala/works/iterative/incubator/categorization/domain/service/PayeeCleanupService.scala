...
trait PayeeCleanupService:
    /** Clean up a payee name using existing rules or LLM generation.
      *
      * This is the main method for payee cleanup. It first attempts to match the original payee
      * name against approved rules. If no rule matches, it falls back to LLM for generating a
      * cleaned name and possibly suggesting new rules.
      *
      * @param original
      *   The original payee name from the transaction
      * @param context
      *   Additional transaction context to aid in cleanup (e.g., counterAccount, message)
      * @return
      *   A task resulting in a PayeeCleanupResult with cleaned name and any rules applied/generated
      */
    def cleanupPayee(
        original: String,
    ...
    /** Get all rules with pending status that need approval.
      *
      * @return
      *   A task resulting in a sequence of pending rules
      */
    ...
    /** Get all approved rules.
      *
      * @return
      *   A task resulting in a sequence of approved rules
      */
    ...
    /** Approve a pending rule, optionally with modifications.
      *
      * @param ruleId
      *   The ID of the rule to approve
      * @param modifications
      *   Optional modifications to the rule before approving
      * @return
      *   A task resulting in the approved rule
      */
    def approveRule(
        ruleId: String,
    ...
    /** Reject a pending rule.
      *
      * @param ruleId
      *   The ID of the rule to reject
      * @param reason
      *   Optional reason for rejection (for auditing)
      * @return
      *   A task resulting in Unit
      */
    def rejectRule(
        ruleId: String,
    ...
    /** Provide feedback on rule application.
      *
      * This method is used to track rule performance over time. When a user approves or corrects a
      * cleaned payee name, this feedback is used to update the rule's success metrics.
      *
      * @param ruleId
    ...
    /** Find rules that match a given payee name.
      *
      * @param payeeName
      *   The payee name to match against rules
      * @return
      *   A task resulting in a sequence of matching rules
      */
    ...
    /** Create a new rule manually (by a human).
      ...
      * @param patternType
      *   The type of pattern (exact, contains, starts with, regex)
      * @param replacement
      *   The replacement text to use
      * @return
      *   A task resulting in the newly created rule (auto-approved)
      */
    def createRule(
        pattern: String,
