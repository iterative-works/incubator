...
/** Service for managing Fio Bank accounts
  *
  * This service provides operations for creating and managing Fio Bank accounts, including setting
  * initial bookmarks to avoid issues with historical data retrieval.
  *
...
trait FioAccountService:
    /** Create a new Fio Bank account with initial bookmark
      *
      * This method creates a new Fio account and automatically sets a bookmark (zarážka) to avoid
      * issues with retrieving historical data older than 90 days.
      *
      * @param sourceAccountId
      *   The source account ID to associate with this Fio account
      * @param token
      *   The Fio API token
      * @return
...
class FioAccountServiceLive(
    ...
    override def createAccount(sourceAccountId: Long, token: String): Task[Long] =
        for
            ...
            encryptedToken <- ZIO.attempt(
                works.iterative.incubator.fio.infrastructure.security.FioTokenManagerLive.encrypt(
                    token,
                    securityConfig.encryptionKey
                )
            )
                .catchAll(error =>
                    ZIO.fail(new RuntimeException(s"Failed to encrypt token: ${error.getMessage}"))
                )
            ...
            _ <- setBookmark(accountId, LocalDate.now.minusDays(60))
                .catchAll(error =>
                    ZIO.logWarning(
                        s"Failed to set initial bookmark for account $accountId: ${error.getMessage}"
                    ) *>
                        ZIO.unit
                )
        yield accountId
...
object FioAccountService:
    ...
    def createAccount(
        ...
        token: String
    ): ZIO[FioAccountService, Throwable, Long] =
    ...
    /** Layer for the service
      */
    val layer: ZLayer[
        FioAccountRepository & FioTokenManager & FioClient & works.iterative.incubator.fio.infrastructure.security.FioSecurityConfig,
        Nothing,
        FioAccountService
    ] =
        ZLayer {
            for
                ...
                securityConfig <- ZIO.service[
                    works.iterative.incubator.fio.infrastructure.security.FioSecurityConfig
                ]
