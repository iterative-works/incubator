...
/** Port interface for interacting with the Transaction Management Context
  *
  * This port defines the interface that the Fio Bank Context needs from the Transaction Management
  * Context. It allows the Fio Bank Context to save transactions without directly depending on the
  * Transaction Management Context's implementation details.
  *
