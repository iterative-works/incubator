...
/** Represents the state of Fio transaction imports Used to track last imported transaction ID and
  * timestamp
  *
...
trait FioImportStateRepository:
    /** Get the import state for a specific source account
      ...
      *   The source account ID to get import state for
      * @return
      *   The import state if it exists
      */
