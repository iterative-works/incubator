...
class DefaultTransactionProcessor(
    ...
    /** Initialize processing state for newly imported transactions
      *
      * This method finds transactions that don't have a corresponding processing state and creates
      * an initial processing state for each of them.
      */
    ...
    /** Process transactions that are in the imported state
      *
      * In a full implementation, this would apply AI categorization or other business logic to move
      * transactions from imported to categorized state. For now, it's a simplified placeholder.
      */
                ...
                for
                    ...
                    updatedCount <- transactionOpt match
                        ...
                        case Some(transaction) =>
                            ...
                            val suggestedPayee =
                                transaction.counterBankName.orElse(transaction.message)
    ...
    /** Find transactions ready for submission to YNAB
      */
    override def findTransactionsReadyForSubmission()
        : Task[Seq[(Transaction, TransactionProcessingState)]] =
        for
            ...
            pairs <-
                ZIO.foldLeft(readyStates)(Seq.empty[(Transaction, TransactionProcessingState)]) {
                    (acc, state) =>
                        for
                            transactionOpt <- transactionRepository.load(state.transactionId)
