...
/** Implementation of YnabTransactionImportService
  ...
  * @param accountMappingRepository
        ...
        for
            ...
            mapping <- ZIO.fromOption(mappingOpt)
                .orElseFail(new RuntimeException(
                    s"No YNAB account mapping found for source account $sourceAccountId"
                ))

            // Only proceed if the mapping is active
            _ <- ZIO.when(!mapping.active) {
                ZIO.fail(new RuntimeException(
                    s"YNAB account mapping for source account $sourceAccountId is not active"
                ))
            }
            ...
            importResults = results.map { case (transaction, id) =>
                val transactionId = transaction.id.getOrElse(
                    transaction.importId.getOrElse(UUID.randomUUID().toString)
                )
