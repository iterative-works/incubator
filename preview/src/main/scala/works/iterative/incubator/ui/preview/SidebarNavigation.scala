...
/** Sidebar navigation component for the preview server Provides links to all available component
  * previews
  */
object SidebarNavigation:
    ...
    /** Render the sidebar navigation
      *
      * @param currentPath
      *   The current path to highlight the active link
      * @return
      *   Scalatags fragment with the navigation structure
      */
                        ...
                        category.items.map { item =>
                            li(
                                a(
                                    href := item.path,
                                    cls := s"sidebar-link ${
                                            if currentPath == item.path then "active" else ""
                                        }",
                                    item.name
                                )
                            )
