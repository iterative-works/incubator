...
object PreviewServer:
    ...
    def getPreviewServerConfig: ZIO[Any, Throwable, BlazeServerConfig] =
        for
            ...
            host <- ZIO.attempt(Option(java.lang.System.getenv("BLAZE_HOST")).getOrElse("localhost"))
    ...
    def serve(registry: PreviewModuleRegistry): ZIO[HttpServer & Scope, Throwable, Unit] =
        for
            ...
            _ <- ZIO.logInfo(s"Preview server running at http://${address.getHostAddress}:${config.port}/preview")
        yield ()
end PreviewServer