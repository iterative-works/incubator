...
class TransactionViewExample:
    ...
    def listView(
        ...
        states: List[TransactionProcessingState]
    ): TypedTag[String] =
        ...
        def statusBadge(status: TransactionStatus): TypedTag[String] = status match
            case TransactionStatus.Imported =>
                span(cls := "bg-yellow-200 text-yellow-800 py-1 px-2 rounded-full text-xs")(
                    "Imported"
                )
            case TransactionStatus.Categorized =>
                span(cls := "bg-blue-200 text-blue-800 py-1 px-2 rounded-full text-xs")(
                    "Categorized"
                )
            case TransactionStatus.Submitted =>
                span(cls := "bg-green-200 text-green-800 py-1 px-2 rounded-full text-xs")(
                    "Submitted"
                )
                        ...
                        transactions.map { tx =>
                            ...
                            tr(cls := "border-b border-gray-200 hover:bg-gray-100")(
                                ...
                                td(cls := "py-3 px-6 text-left")(
                                    tx.userIdentification.getOrElse("No description")
                                ),
                                td(cls := "py-3 px-6 text-right")(formatAmount(tx.amount)),
                                td(cls := "py-3 px-6 text-left")(
                                    s"${tx.counterAccount.getOrElse("-")}/${tx.counterBankCode.getOrElse("-")}"
                                ),
                                td(cls := "py-3 px-6 text-center")(
                                    state.map(s => statusBadge(s.status)).getOrElse(
                                        span(
                                            cls := "bg-gray-200 text-gray-800 py-1 px-2 rounded-full text-xs"
                                        )("Unknown")
                                    )
                                ),
                                td(cls := "py-3 px-6 text-right")(
                                    a(href := "#", cls := "text-blue-500 hover:underline mr-2")(
                                        "View"
                                    ),
                                    a(href := "#", cls := "text-yellow-500 hover:underline")(
                                        "Process"
                                    )
                                )
                            )
    ...
    def emptyView(): TypedTag[String] =
        div(cls := "container mx-auto")(
            ...
            div(cls := "bg-white shadow rounded p-8 text-center")(
                ...
                a(href := "#", cls := "px-4 py-2 bg-blue-500 text-white rounded")(
                    "Import Transactions"
                )
            )
        )
    ...
    def warningView(
        ...
        div(cls := "container mx-auto")(
            ...
            // Warning messages
            div(
                cls := "bg-orange-100 border border-orange-400 text-orange-700 px-4 py-3 rounded mb-4"
            )(
                    ...
                    tbody(
                        transactions.map {
                            tx =>
                                ...
                                tr(
                                    cls := s"border-b border-gray-200 hover:bg-gray-100 ${if hasPotentialWarning then "bg-orange-50" else ""}"
                                )(
                                    td(cls := "py-3 px-6 text-left")(tx.date.format(dateFormatter)),
                                    td(cls := "py-3 px-6 text-left")(
                                        tx.userIdentification.getOrElse("No description")
                                    ...
                                    td(cls := "py-3 px-6 text-right")(
                                        ...
                                        if hasPotentialWarning then
                                            a(
                                                href := "#",
                                                cls := "text-orange-500 hover:underline mr-2"
                                            )("Resolve Warning")
                                        else
                                            a(
                                                href := "#",
                                                cls := "text-yellow-500 hover:underline"
                                            )("Process")
                                    )
                                )
                        }
